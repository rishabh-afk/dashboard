...
const HelpDeskAgentModal = ({
  ...
  const [userId, setUserId] = useState<any>(data?._id ?? "");
  const [skills, setSkills] = useState<any>(data?.skills ?? []);
  const [active, setActive] = useState<any>(data?.availability ?? false);
  ...
  useEffect(() => {
    ...
    if (users.length === 0 && !data?._id) fetchUsers();
  }, [users.length, data?._id]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      let url = "";
      if (data?._id) url = `${endpoints["HelpDeskAgent"].update}`;
      else url = `${endpoints["HelpDeskAgent"].create}`;
      const response: any = data?._id
        ? await Put(url, updated)
        : await Post(url, updated);
      if (response?.success) {
        const fetchUrl = `${endpoints["HelpDeskAgent"].fetchAll}`;
        ...
        <div className="mt-5">
          <div className="grid grid-cols-2 gap-5">
            ...
            <ToggleButton
              keyUpdate={true}
              setState={setActive}
              field={{ ...fieldToggle, value: active }}
            />
          </div>
...
export default HelpDeskAgentModal;
